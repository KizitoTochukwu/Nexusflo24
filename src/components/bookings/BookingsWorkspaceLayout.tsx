...
import {
  BOOKINGS_PRIMARY_NAV, BOOKINGS_SECONDARY_NAV, BOOKINGS_ALL_NAV, resolveBookingsSection,
} from "@/lib/bookings/nav";

/**
 * Shared shell for every Bookings page: breadcrumbs plus a secondary navigation
 * bar so users move between booking sections without leaving the module.
 */
const BookingsWorkspaceLayout = () => {
  const workspaceId = useWorkspaceId();
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const isMobile = useIsMobile();

  const active = resolveBookingsSection(pathname);
  const to = (path: string) => `/dashboard/${workspaceId}/${path}`;
  const moreActive = BOOKINGS_SECONDARY_NAV.some((i) => i.key === active?.key);
        ...
        <div className="sticky top-0 z-10 w-full space-y-2 border-b border-border/50 bg-background/90 pb-3 pt-2 backdrop-blur-md supports-[backdrop-filter]:bg-background/75">
            ...
            <BreadcrumbList className="text-xs">
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <NavLink to={to("bookings")} className="flex items-center gap-1.5 font-medium">
          ...
          {isMobile ? (
            <Select
              value={active?.key ?? ""}
              onValueChange={(key) => {
                const item = BOOKINGS_ALL_NAV.find((i) => i.key === key);
                if (item) navigate(to(item.path));
              }}
            >
              <SelectTrigger className="w-full rounded-xl" aria-label="Bookings section">
                <SelectValue placeholder="Choose a section" />
              </SelectTrigger>
              <SelectContent className="z-[70]">
                <SelectGroup>
                  <SelectLabel>Sections</SelectLabel>
                  {BOOKINGS_PRIMARY_NAV.map((i) => (
                    <SelectItem key={i.key} value={i.key}>{i.label}</SelectItem>
                  ))}
                </SelectGroup>
                <SelectGroup>
                  <SelectLabel>More</SelectLabel>
                  {BOOKINGS_SECONDARY_NAV.map((i) => (
                    <SelectItem key={i.key} value={i.key}>{i.label}</SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
            ...
            >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="z-[70] w-56 rounded-xl bg-popover shadow-lg">
                  {BOOKINGS_SECONDARY_NAV.map((item) => (
                    <DropdownMenuItem
                      key={item.key}
...
export default BookingsWorkspaceLayout;
