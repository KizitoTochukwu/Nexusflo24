-- Split the single trigger function into two table-specific ones so that
-- NEW.channel / NEW.provider references don't fail when the trigger fires
-- on whatsapp_settings (which has neither column).
